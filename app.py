...
def build_excel(report_df, compliant, attention, failed, total, elapsed):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        for sname in ["Audit Report", "Failed Items", "Needs Attention", "Summary"]:
...
def build_zip(results):
    """Pack COMPLIANT and ATTENTION images into a ZIP named ProductTitle.MAIN.jpg"""
    zip_buf    = BytesIO()
    count      = 0
    seen_names = {}
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for r in results:
            if r.get("_image_bytes") and r.get("Status") in ("COMPLIANT", "ATTENTION"):
                ...
                zf.writestr(fname, r["_image_bytes"])
                count += 1
    ...
    try:
        ...
        if missing:
            ...
            if st.button(f"🚀  Start Audit  ({len(df)} products)", use_container_width=True):
                ...
                priority    = ["Product Title", "Image URL", "Status", "Audit Notes", "Resolved URL"]
                ...
                with dl1:
                    ...
                    st.markdown("""
4 sheets inside:
- **Audit Report** — every product with status & notes
- **Summary** — pass rate and run details
- **Failed Items** — re-uploadable list
- **Needs Attention** — manual review queue
""")
                    st.download_button(
...
- Named `SKU.MAIN.jpg` — upload-ready
- White background · JPEG · 72 DPI
- Minimum 1000px on longest side
- Unzip and drag straight into Seller Central
""")
                    st.download_button(
